...
namespace {
  class MyAAEval : public FunctionPass {
    unsigned NoAlias, MayAlias, PartialAlias, MustAlias;
    unsigned NoModRef, Mod, Ref, ModRef;
    unsigned analyzedFunc;

  public:
    static char ID; // Pass identification, replacement for typeid
    MyAAEval() : FunctionPass(ID) {
 //     initializeMyAAEvalPass(*PassRegistry::getPassRegistry());
    }
...
char MyAAEval::ID = 0;
RegisterPass<MyAAEval> myaaeval("my-aa-eval", "SOS - Exhaustive Alias Analysis Precision Evaluator", false, true);
//INITIALIZE_PASS_BEGIN(MyAAEval, "aa-eval",
//                "Exhaustive Alias Analysis Precision Evaluator", false, true)
//INITIALIZE_AG_DEPENDENCY(AliasAnalysis)
//INITIALIZE_PASS_END(MyAAEval, "aa-eval",
//                "Exhaustive Alias Analysis Precision Evaluator", false, true)
//
//FunctionPass *llvm::createMyAAEvalPass() { return new MyAAEval(); }
...
bool MyAAEval::runOnFunction(Function &F) {
  AliasAnalysis &AA = getAnalysis<AliasAnalysis>();

  analyzedFunc++;
  errs() << "My AA Eval: " << analyzedFunc << " - "<< F.getName().str() << " ... \n";
...
//    }
 ...
 //     errs() << "MyAAEval: AA.alias(*I1, I1Size, *I2, I2Size): *I1 = " << **I1 << " *I2 = " << **I2 << "\n";
      const LoadInst *L1 = dyn_cast<LoadInst>(*I1);
      const LoadInst *L2 = dyn_cast<LoadInst>(*I2);
      if (L1 != nullptr) {
        errs() << "I1 Location: " << AA.getLocation(L1).Ptr << "\n";
      }
      if (L2 != nullptr) {
        errs() << "I2 Location: " << AA.getLocation(L2).Ptr << "\n";
      }

      if (L1 != nullptr) {
          errs() << "same locations: \n";
          switch (AA.alias(L1, L1)) {
...
//        PrintResults("PartialAlias", PrintPartialAlias, *I1, *I2, F.getParent());
 ...
 // errs() << "MyAAEval: 3\n";
/// Zhiyuan: this part is not useful for evaluating alias analysis
//  if (EvalTBAA) {
...
bool MyAAEval::doFinalization(Module &M) {
    int totalFunc = 0;
    for (Module::iterator F = M.begin(), E = M.end(); F != E; F++) {
        totalFunc++;
    }
    errs() << "[My AA Eval]Functions Number in the module: " << totalFunc << "\n";
    errs() << "[My AA Eval]runOnFunction runs Number: " << analyzedFunc << "\n";
